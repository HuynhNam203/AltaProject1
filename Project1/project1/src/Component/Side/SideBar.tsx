...
const items: MenuProps["items"] = [
  {
    label: <Link to="/Roles">Quản lý vai trò</Link>,
    key: "0",
  },
  {
    label: <Link to="/QuanlyTK">Quản lý tài khoản</Link>,
    key: "1",
  },
  {
    label: <Link to="/UserHistory">Nhật kí người dùng</Link>,
    key: "2",
  },
];
          ...
          <ul className="Side">
            {SideData.map((item, index) => {
              return (
                <li key={index}>
                  <Link to={item.path}>
                    <span className="PropIcon">{item.icon}</span>
                    <span className="PropName">{item.title}</span>
                    <Dropdown menu={{ items }} trigger={["hover"]}><span className="PropMenu">{item.menuicon}</span></Dropdown>
                  </Link>
                </li>
              );
            })}{" "}
